...
def connectDB():
    print('连接到mysql服务器...')
    # 打开数据库连接
    db = pymysql.connect("localhost", "root", "123456", "movies", charset="utf8")
    print('连接上了!')
    return db


def insertMovie(db, name, path, time, brief, score, type_id):
    cursor = db.cursor()

    sql = "INSERT INTO movie(movie_name, img_path, movie_time, brief, score, type_id ) values('%s', '%s', '%s', '%s', " \
          "'%s', %d);" % (
              name, path, time, brief, score, type_id)
    try:
        print(sql)
        cursor.execute(sql)
        db.commit()
